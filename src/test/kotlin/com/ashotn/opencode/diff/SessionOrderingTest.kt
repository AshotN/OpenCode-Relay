...
class SessionOrderingTest {
    ...
    fun `session order is determined by server timestamps, not by reconcile completion order`() {
        ...
        for ((sid, relPath, content) in listOf(
            ...
            val event = com.ashotn.opencode.ipc.OpenCodeEvent.SessionDiff(
                sessionId = sid,
                files = listOf(com.ashotn.opencode.ipc.OpenCodeEvent.SessionDiffFile(
                    file = absPath,
                    before = "",
                    after = "ai content",
                    additions = 1,
                    deletions = 0,
                    status = com.ashotn.opencode.ipc.SessionDiffStatus.MODIFIED,
                )),
            )
